...
if (function_exists('is_plugin_active') && is_plugin_active('learndash-course-grid/learndash_course_grid.php') && isset($shortcode_atts['course_grid']) && '' !== $shortcode_atts['course_grid']) {
    ...
    $new_lesson_count = learndash_get_lesson_list($course_id, array('num' => -1));


    $pendingArr = array();
    $pendingLessonArr = array();
    $completeArr = array();
    $completeLessonArr = array();
    foreach ($new_lesson_count as $lesson) {
        $completeLessonArr[] = $lesson->ID;
        if (!learndash_is_item_complete($lesson->ID)) {
            $pendingLessonArr[] = $lesson->ID;
        }
        $lesson_topics = learndash_get_topic_list($lesson->ID);
        if ($lesson_topics) {
            foreach ($lesson_topics as $to) {
                $completeArr[] = $to->ID;
                if (!learndash_is_item_complete($to->ID)) {
                    $pendingArr[] = $to->ID;
                }
            }
        }
    ...
    $taskUrl = '';
    //$lessionUrl = the_permalink();
    if ($pendingArr && isset($pendingArr[0])) {
        if ($progress['percentage'] >= 1 && $progress['percentage'] < 100) {
            $taskUrl = learndash_get_step_permalink($pendingArr[0], $course_id);
        } else {
            $taskUrl = learndash_get_step_permalink($pendingArr[0], $course_id);
        }
    } else {
        //check if lessions are available
        if ($pendingLessonArr && isset($pendingLessonArr[0])) {
            if ($progress['percentage'] >= 1 && $progress['percentage'] < 100) {
                $taskUrl = learndash_get_step_permalink($pendingLessonArr[0]);
            } else if ($progress['percentage'] === 0) {
                $taskUrl = learndash_get_step_permalink($pendingLessonArr[0]);
            }
        }
        //echo 'hello there '.$progress['percentage'];
        if ($progress['percentage'] >= 100) {
            if ($completeArr && isset($completeArr[0])) {
                $taskUrl = learndash_get_step_permalink($completeArr[0], $course_id);
            } else {
                if ($completeLessonArr && isset($completeLessonArr[0])) {
                    $taskUrl = learndash_get_step_permalink($completeLessonArr[0]);
                }
    ...
    ?>
    <div class="ld_course_grid col-sm-<?php echo $smcol; ?> col-md-<?php echo $col; ?> <?php echo esc_attr($grid_class); ?> bb-course-item-wrap grid-sec">
        ...
        <div class="bb-cover-list-item <?php echo $class_price_type; ?> <?php echo $class_content_type; ?>">
			<?php if ($shortcode_atts['show_thumbnail'] == 'true'): ?>
                    ...
                    <div class="bb-course-cover">
                        <a title="<?php the_title_attribute();?>" href="<?php echo $taskUrl; ?>" class="bb-cover-wrap">
    ...
    $topic_count = learndash_get_topic_list(get_the_ID(), array('num' => -1));
    $topics_count = sizeof($topic_list);
    ...
    foreach ($new_lesson_count as $lesson) {
        $lesson_topics = learndash_get_topic_list($lesson->ID);
        if ($lesson_topics) {
            $new_topics_count += sizeof($lesson_topics);
        }
    }
    ...
    ?>
                    ...
                    <h2 class="bb-course-title"><a href="<?php echo $taskUrl; ?>"><?php the_title();?></a></h2>
                    <div class="row">
    ...
    ?>
                    		...
                    		<div class="col-md-6 pull-right" style="padding:0px; text-align: right;">
                    			<?php
$tags = get_the_tags(get_the_ID());
    ...
    $tagStr = '';
    if (count($tagsName) > 0) {
        $tagStr = implode(', ', $tagsName);
    }

    /*if( $lessons_count > 0 ) { ?>
    <div class="course-lesson-count"><?php echo $total_lessons?><?php
    echo $total_topics > 0 ?  ',<br> '.$total_topics : '';
    ?></div>
    <?php } else {
    ...
    ?>
								<div class="course-lesson-count"><?php echo $tagStr ?></div>
                    		</div>
                    	</div>
                   <!--  	<div class="col-md-12 cou-short-c">
                    	   <?php the_excerpt();?>
                    	</div> -->
                    	<div class="col-md-12 cou-btn">
                    		<?php
								if ($progress['percentage'] >= 100) {
									echo '<a href="' . esc_attr($taskUrl) . '">Completed</a>';
								} else {

									$incompletedId = '';
									foreach ($new_lesson_count as $lesson) {
										if (!learndash_is_item_complete($lesson->ID)) {
											$incompletedId = $lesson->ID;
											break;
										}
										$lesson_topics = learndash_get_topic_list($lesson->ID);
										if ($lesson_topics) {
