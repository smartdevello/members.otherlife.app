                        ...
                        <div class="thumbnail-container <?php echo esc_attr($thumb_mode); ?>">
                            <div class="bb-course-video-overlay">
                                <div>
                                    ...
                                    <div><?php _e('Preview this', 'buddyboss-theme');?>
                                        <?php echo LearnDash_Custom_Label::get_label('course'); ?></div>
                                </div>
                            </div>
                            <?php
    ...
    foreach ($course_members as $course_member):
        if ($count > 2) {
            break;
                        ...
                        <img class="round"
                            src="<?php echo get_avatar_url((int) $course_member, array('size' => 96)); ?>" alt="" /><?php
    $count++;
    endforeach;?>
                    </span>
...
error_log('course_price_type ' . $course_price_type);
if ($is_enrolled) {
    error_log('Is Enrolled');
} else {
    error_log('Not Enrolled');
}
...
if ($course_price_type == 'open' || $course_price_type == 'free') {
    ...
    ?>
                    <div class="learndash_join_button testLogin <?php echo $btn_advance_class; ?>">
        ...
        if ($course_price_type == 'free' && false === $is_enrolled) {
            ...
            ?>
                    <div class="learndash_join_button   testAdvanceform <?php echo $btn_advance_class; ?>">
                        <form method="post">
            ...
            ?>
                    <div class="learndash_join_button testAdvancelink <?php echo $btn_advance_class; ?>">
    ...
    if ($course_price_type == 'open') {
        ?>
                    <span
                        class="bb-course-type bb-course-type-open"><?php _e('Open Registration', 'buddyboss-theme');?></span><?php
} else {
        ?>
...
if ('' === $course_price && $course_price_type == 'subscribe') {
                ?>
                        <span
                            class="bb-course-type bb-course-type-subscribe"><?php _e('Free', 'buddyboss-theme');?></span>
                        <?php
} else {
            ...
            //if ( !empty( $course_price_billing_p3 ) ) { ?>
                        <span class="course-bill-cycle"> / <?php echo $recuring . ' ' . $course_price_billing_t3; ?>
                        </span><?php
//} ?>
                    </span>
                    <?php
...
if (sizeof($lesson_count) > 0 || $topics_count > 0 || $course_quizzes_count > 0 || $course_certificate) {?>
    ...
    'buddyboss-theme');?></h4>
                    <ul class="bb-course-volume-list">
                        ...
                        <li>
                            <i
                                class="bb-icon-l bb-icon-certificate"></i><?php echo LearnDash_Custom_Label::get_label('course'); ?> <?php _e('Certificate',
    'buddyboss-theme');?>
                        </li>
                        <?php }?>
                    </ul>
